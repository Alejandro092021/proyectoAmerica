        ...
        <div class="container relative left-0 z-50 flex w-2/4 h-auto">
          <div class="flex justify-center items-center w-6 h-full m-2">
            <div class="hidden md:block">
              <svg
                viewBox="0 0 24 24"
                class="w-6 h-6 cursor-pointer text-gray-400 dark:text-white transition duration-500 ease-out"
                @click="abriMenu"
                :class="estadoMenuIcon ? 'rotate-180' : ''"
              >
                <path
                  fill="currentColor"
            ...
            <div class="flex md:hidden">
              <svg
                iewBox="0 0 20 20"
                class="w-6 h-6 cursor-pointer text-gray-400 dark:text-white"
                fill="currentColor"
                @click="abriMenuMobil"
                :class="estadoMenuIcon ? 'rotate-180' : ''"
              >
            ...
            <jet-dropdown align="right" width="48" >
              <template #trigger>
                <button
                  ...
                  class="dark:bg-gray-700 flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition"
                >
                  <img
                ...
                <span v-else class="inline-flex rounded-md">
                  <button
                    type="button"                    
                    class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition dark:bg-gray-700 dark:text-white"
                  >
...
export default defineComponent({
  ...
  methods: {
    abriMenu() {
      this.$emit("menu");
      this.estadoMenuIcon = !this.estadoMenuIcon;
    },
    abriMenuMobil() {
      this.$emit("mobil");
      this.estadoMenuIcon = !this.estadoMenuIcon;
